...
import { regionColorMatchPairs } from "../domain/regions";
...
/** 訪問済の塗り色式: premium時は地方ブロック別(SPEC §11)、それ以外は単色テーマ。 */
function visitedColorExpr(premium: boolean): maplibregl.DataDrivenPropertyValueSpecification<string> {
  if (!premium) return THEME as unknown as maplibregl.DataDrivenPropertyValueSpecification<string>;
  return ["match", ["get", "pref"], ...regionColorMatchPairs(), THEME] as unknown as maplibregl.DataDrivenPropertyValueSpecification<string>;
}

function baseColorExpr(premium: boolean): maplibregl.DataDrivenPropertyValueSpecification<string> {
  return ["case", ["boolean", ["feature-state", "visited"], false], visitedColorExpr(premium), UNVISITED] as unknown as maplibregl.DataDrivenPropertyValueSpecification<string>;
}
...
export function MapView({ onSelect, isVisited, onReady, premium }: Props) {
  ...
  useEffect(() => {
    ...
    map.on("load", async () => {
      ...
      map.addLayer({
        ...
        filter: ["!", ["has", "point_count"]],
        paint: {
          "circle-color": visitedColorExpr(premium),
          "circle-blur": 1,
      ...
      map.addLayer({
        ...
        filter: ["!", ["has", "point_count"]],
        paint: {
          "circle-color": baseColorExpr(premium),
          "circle-radius": [
            "interpolate",
            ["linear"],
            ["zoom"],
  ...
  // premium切り替え時に塗り色式を更新(地方ブロック別 ⇔ 単色テーマ)
  useEffect(() => {
    const map = mapRef.current;
    if (!map) return;
    const apply = () => {
      map.setPaintProperty(GLOW, "circle-color", visitedColorExpr(premium));
      map.setPaintProperty(BASE, "circle-color", baseColorExpr(premium));
    };
    if (map.isStyleLoaded() && map.getLayer(BASE)) apply();

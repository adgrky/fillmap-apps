...
import { REGION_COLORS, type Region } from "../domain/regions";
...
export function App() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("michimap:premium-unlocked", onUnlocked);
    return () => window.removeEventListener("michimap:premium-unlocked", onUnlocked);
  }, []);
  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      ...
      {/* 地方ブロック別カラーの凡例(premium時のみ、railmap同様) */}
      {premium && tab === "map" && (
        <div className="absolute right-3 top-[180px] z-10 flex flex-col gap-1 rounded-xl bg-[#11151c]/90 p-2 text-xs backdrop-blur">
          {(Object.entries(REGION_COLORS) as [Region, string][]).map(([region, color]) => (
            <div key={region} className="flex items-center gap-1.5">
              <span className="h-2.5 w-2.5 rounded-full" style={{ background: color }} />
              <span className="text-[#c5cbd6]">{region}</span>
            </div>
          ))}
        </div>
      )}

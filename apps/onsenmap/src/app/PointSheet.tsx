// 温泉タップ時のボトムシート(SPEC §E2/§A6)。入湯トグル+入湯日/回数/メモ。
import type { Bath, OnsenInfo } from "../domain/types";

type Props = {
  info: OnsenInfo;
  bath: Bath | undefined; // undefined = 未入湯
  onToggle: () => void;
  onUpdate: (patch: Partial<Bath>) => void;
  onClose: () => void;
};

export function PointSheet({ info, bath, onToggle, onUpdate, onClose }: Props) {
  const bathed = Boolean(bath);
    ...
    <div className="absolute inset-x-0 bottom-0 z-20 rounded-t-2xl bg-[#11151c] p-4 pb-6 shadow-2xl">
      ...
      <button
        ...
        className={`w-full rounded-xl py-3 text-base font-bold transition ${
          bathed ? "bg-[#1e2530] text-[#8b93a3]" : "bg-rose-400 text-[#0b0e14]"
        }`}
      >
        {bathed ? "入湯を取り消す" : "♨ 浸かった！"}
      </button>
      ...
      {bathed && bath && (
        <div className="mt-4 space-y-3">
          <div className="flex items-center gap-3">
            <label className="w-16 text-xs text-[#8b93a3]">入湯日</label>
            <input
              type="date"
              value={bath.firstDate ?? ""}
          ...
          <div className="flex items-center gap-3">
            <label className="w-16 text-xs text-[#8b93a3]">入湯回数</label>
            <div className="flex items-center gap-3">
              <button
                onClick={() => onUpdate({ count: Math.max(1, bath.count - 1) })}
                className="h-8 w-8 rounded-lg bg-[#1e2530] text-lg leading-none text-[#e8ecf4]"
              >
                −
              </button>
              <span className="w-6 text-center text-sm font-semibold">{bath.count}</span>
              <button
                onClick={() => onUpdate({ count: bath.count + 1 })}
                className="h-8 w-8 rounded-lg bg-[#1e2530] text-lg leading-none text-[#e8ecf4]"
              >
                ＋
              </button>

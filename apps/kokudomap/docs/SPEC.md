...
## §11 国道トリビアタグ(プレミアム機能、2026-06-26実装)
- 「酷道」ランキング(個人サイトによる5段階評価)は著作権上の懸念(転載に許可が必要と明記)があるため不採用。事実ベースの公式データのみ採用。
- 海上国道(29路線): 出典 Wikipedia「海上国道」(CC BY-SA、一般国道の路線を指定する政令に基づく事実)。
- 点線国道(交通不能区間のある16路線): 出典 国土交通省 道路データブック2025「10-3 一般国道における交通不能区間の状況」。
- データは `domain/roadTags.ts` に静的定義(路線数が少なく更新頻度も低いため、別ファイル取得は行わない)。premium時のみLineSheetにバッジ表示(🌊海上国道 / ⚠️点線国道)。

## 残課題(v2)
- 旧道タグ: 区間単位の旧道判定にはルート単位ではなく区間ごとの新旧線形データが必要(現データ構造では対応不可、v2で要検討)
- 都道府県別達成率(OSM relationに県情報が無く現状pref空)。データ年1回更新。

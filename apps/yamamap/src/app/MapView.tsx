...
const EXTRA_SRC = "extra-peaks";
const EXTRA_LAYER = "extra-peaks-points";
...
type Props = {
  ...
  premium: boolean;
  onSelectExtraPeak: (p: ExtraPeak) => void;
};
...
export function MapView({ onSelect, isVisited, onReady, premium, onSelectExtraPeak }: Props) {
  const containerRef = useRef<HTMLDivElement>(null);
  const mapRef = useRef<maplibregl.Map | null>(null);
  const coordsById = useRef<Map<string, [number, number]>>(new Map());
  const rippleRaf = useRef<number>(0);
  const onSelectExtraPeakRef = useRef(onSelectExtraPeak);
  onSelectExtraPeakRef.current = onSelectExtraPeak;
  ...
  // 二百名山・三百名山レイヤー(プレミアム, §11): premium が立った時だけ取得・追加。
  useEffect(() => {
    const map = mapRef.current;
    if (!map) return;

    const teardown = () => {
      if (map.getLayer(EXTRA_LAYER)) map.removeLayer(EXTRA_LAYER);
      if (map.getSource(EXTRA_SRC)) map.removeSource(EXTRA_SRC);
    };
    ...
    const setup = async () => {
      if (map.getSource(EXTRA_SRC)) return;
      const res = await fetch("./data/extra_peaks.json");
      const list: ExtraPeak[] = await res.json();
      const fc = {
      ...
      map.addSource(EXTRA_SRC, { type: "geojson", data: fc });
      map.addLayer({
        id: EXTRA_LAYER,
        type: "circle",
        source: EXTRA_SRC,
        paint: {
          "circle-radius": 4,
          "circle-color": ["match", ["get", "rank"], "三百名山", "#fcd34d", "#67e8f9"],
          "circle-stroke-width": 1,
          "circle-stroke-color": "#0b0e14",
          "circle-opacity": 0.85,
        },
      });
      map.on("click", EXTRA_LAYER, (e) => {
        const f = e.features?.[0];
        if (!f?.properties) return;
        onSelectExtraPeakRef.current(f.properties as ExtraPeak);
      });
      map.on("mouseenter", EXTRA_LAYER, () => (map.getCanvas().style.cursor = "pointer"));
      map.on("mouseleave", EXTRA_LAYER, () => (map.getCanvas().style.cursor = ""));
    };

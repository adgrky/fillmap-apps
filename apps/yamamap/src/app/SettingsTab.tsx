...
export function SettingsTab({ premium, onPremiumUnlocked }: Props) {
  ...
  const handleRedeem = async () => {
    ...
    const ok = await redeemPremiumCode("yamamap.v1", code, UNLOCK_HASH);
    if (ok) {
      onPremiumUnlocked();
      setCode("");
    } else {
      ...
      <div className="mx-auto max-w-md space-y-4 px-4 pt-4">
          ...
          {premium ? (
            ...
            <div className="space-y-2">
              <p className="text-sm text-[#8b93a3]">
                買い切りで広告を非表示にし、プレミアム機能(二百名山・三百名山)を解放できます。
              </p>
              <button
                onClick={() => purchasePremium()}
                className="block w-full rounded-lg bg-emerald-400 py-3 text-center text-sm font-semibold text-[#0b0e14]"
              >
                💳 広告を削除する(買い切り)
              </button>
              <button
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="rounded-lg bg-emerald-400 px-4 py-2 text-sm font-semibold text-[#0b0e14]"
                >
                  適用
                </button>
              </div>
        ...
        <section className="rounded-2xl bg-[#11151c] p-4 text-xs text-[#8b93a3]">
          ...
          <p className="mb-3 rounded-lg bg-[#1e2530] px-3 py-2 text-[#e8ecf4]">
            登頂記録はすべて端末内にのみ保存されます。決済ページへの遷移を除き、外部サーバーへの送信は行いません。
          </p>
          <p className="font-semibold text-[#8b93a3]">出典</p>
          <ul className="mt-1 space-y-1">
            <li>
              山岳データ(山名・標高・座標): 「日本百選と座標値」(
              <a
                href="https://100sen.cyber-ninja.jp/"
                target="_blank"
                rel="noopener noreferrer"
                className="underline"
              >
              ...
              )をもとに作成。標高は深田久弥『日本百名山』の伝統値。
            </li>
            <li>山名・標高・座標は事実データであり、百名山100座のみを抽出しています。</li>
          </ul>
        </section>
      </div>
    </div>

...
// native(Capacitor)時のみ広告(AdMob)・課金(Google Play)を初期化する(SPEC §14.3/§14.4)。
// 広告ユニット/商品IDは env。premium 確定はグローバルイベントで App に伝える。
if (isNativeApp()) {
  ...
  void initIap({
    productId: (import.meta.env.VITE_IAP_PRODUCT_ID as string) || "railmap.premium",
    storageKey: "railmap.v1",
    onUnlocked: () => window.dispatchEvent(new Event("railmap:premium-unlocked")),
  });
}

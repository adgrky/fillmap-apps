...
## 2026-09-07 都道府県別を実装・実機で3件の不具合を修正
- 完了: ①N03(行政区域2024)交差で meta.lines[].pref / totals.byPref を付与(tools/add_pref.py。47県別を1件ずつDL→即削除しピーク使用量を1県分に抑制)。597/597路線に付与、47県カバー。②Android 15+ の edge-to-edge でステータスバー/ジェスチャーバーとUIが重なる問題を safe-area で修正。③AdMobバナーが下部タブバーを覆いタブが押せない問題を、バナー実高さ購読(onBannerHeightChange)＋nav/パネルの bottom 調整で修正。
- 決定: バナー位置は showBanner の margin では解決不可(プラグインが Android 15+ で WindowInsets リスナーにより margin を上書き。BannerExecutor.java 確認済)。Web側を持ち上げる方式を採用。掲載文の「都道府県別」は実装完了により記載を復帰可能。
- 残課題: Play掲載用スクショ5枚を撮り直し済(`Playストア提出セット/1_塗り鉄/2_画像/スクショ/`)。エミュレータのステータスバー領域に前フレームの描画残りが出るため上部250pxを切って対処。実機撮影ならこの処理は不要。

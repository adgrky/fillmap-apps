// 振り返り年表タブ(SPEC §A4 W5)。firstDate 年別に攻略した城を一覧表示。
import type { CastleMeta, Conquest } from "../domain/types";
...
function buildYearGroups(meta: CastleMeta, visits: Record<string, Conquest>): {
  groups: YearGroup[];
  undated: Entry[];
} {
    ...
    <div className="absolute inset-0 top-[88px] overflow-y-auto pb-4">
      <div className="mx-auto max-w-md space-y-6 px-4 pt-4">
        {groups.map(({ year, months }) => (
          <section key={year}>
            <h2 className="mb-2 text-lg font-bold text-yellow-300">{year}年</h2>
            <div className="space-y-4">
              {months.map(({ month, entries }) => (
                <div key={month}>
                  <div className="mb-1 flex items-baseline gap-2">
                    <h3 className="text-sm font-semibold text-[#e8ecf4]">{Number(month)}月</h3>
                    <span className="text-xs text-[#8b93a3]">{entries.length}城</span>
                  </div>
        ...
        {undated.length > 0 && (
          <section>
            <div className="mb-2 flex items-baseline gap-2">
              ...
              <span className="text-xs text-[#8b93a3]">{undated.length}城</span>
            </div>

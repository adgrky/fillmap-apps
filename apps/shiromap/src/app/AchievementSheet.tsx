// 称号一覧シート(SPEC §A4)。解除済み/未解除を一覧表示。
import { castleAchievements } from "../domain/achievementDefs";
  ...
  return (
    <div className="absolute inset-0 z-30 flex flex-col bg-[#0b0e14]/95 backdrop-blur">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
        {castleAchievements.map((a) => {
          const date = unlocked[a.id];
          const isUnlocked = Boolean(date);
          const isGold = a.id === "tenka";
          return (
            <div
              key={a.id}
              className={`flex items-center gap-4 rounded-xl px-4 py-3 ${
                isUnlocked ? "bg-[#2a260f]" : "bg-[#151a23] opacity-60"
              }`}
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {isUnlocked && (
                  <div className="text-xs text-[#a3892a] mt-1">

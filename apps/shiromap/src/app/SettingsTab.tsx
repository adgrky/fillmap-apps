...
export function SettingsTab({ premium, onPremiumUnlocked }: Props) {
  ...
  const handleRedeem = async () => {
    ...
    const ok = await redeemPremiumCode("shiromap.v1", code, UNLOCK_HASH);
    if (ok) {
      onPremiumUnlocked();
      setCode("");
    } else {
    ...
    <div className="absolute inset-0 top-[88px] overflow-y-auto pb-4">
      <div className="mx-auto max-w-md space-y-4 px-4 pt-4">
        <section className="rounded-2xl bg-[#11151c] p-4">
          ...
          {premium ? (
            <p className="rounded-lg bg-yellow-400/10 p-3 text-sm text-yellow-300">
              購入済みです。広告は表示されません。
            </p>
          ) : (
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="rounded-lg bg-yellow-500 px-4 py-2 text-sm font-semibold text-[#0b0e14]"
                >
                  適用
                </button>
              </div>
        ...
        <section className="rounded-2xl bg-[#11151c] p-4 text-xs text-[#8b93a3]">
          ...
          <p className="mb-3 rounded-lg bg-[#1e2530] px-3 py-2 text-[#e8ecf4]">
            攻略記録はすべて端末内にのみ保存されます。決済ページへの遷移を除き、外部サーバーへの送信は行いません。
          </p>
          <p className="font-semibold text-[#8b93a3]">出典</p>
          <ul className="mt-1 space-y-1">
            <li>
              城データ(名称・所在地・座標): 公益財団法人日本城郭協会「日本100名城」(2006年発表)。
              座標は「日本百選と座標値」(
            ...
            <li>名称・所在地・座標は事実データであり、100名城のみを抽出しています。</li>
          </ul>
        </section>
      </div>
    </div>

...
export function SettingsTab({ premium, onPremiumUnlocked }: Props) {
  ...
  const handleRedeem = async () => {
    ...
    const ok = await redeemPremiumCode("citymap.v1", code, UNLOCK_HASH);
    if (ok) {
      onPremiumUnlocked();
      setCode("");
    } else {
      ...
      <div className="mx-auto max-w-md space-y-4 px-4 pt-4">
          ...
          {premium ? (
            ...
            <div className="space-y-2">
              <p className="text-sm text-[#8b93a3]">
                買い切りで広告を非表示にし、プレミアム機能(消滅自治体)を解放できます。
              </p>
              <button
                onClick={() => purchasePremium()}
                className="block w-full rounded-lg bg-cyan-400 py-3 text-center text-sm font-semibold text-[#0b0e14]"
              >
                💳 広告を削除する(買い切り)
              </button>
              <button
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="rounded-lg bg-cyan-400 px-4 py-2 text-sm font-semibold text-[#0b0e14]"
                >
                  適用
                </button>
              </div>
        ...
        <section className="rounded-2xl bg-[#11151c] p-4 text-xs text-[#8b93a3]">
          ...
          <ul className="mt-1 space-y-1">
            <li>自治体データ: 国土交通省 国土数値情報(行政区域データ N03)をもとに作成</li>
            <li>
              地図タイル: ©{" "}
